...
var format_time="hh:ii:ss";
var format_date_time="yyyy-mm-dd hh:ii:ss";

function setDatePicker(clazz){
	setFormatPicker(clazz,format_date);
}
function setTimePicker(clazz){
	setFormatPicker(clazz,format_time);
}
function setDateTimePicker(clazz){
	setFormatPicker(clazz,format_date_time);
}
function setFormatPicker(clazz,formater){
	$("."+clazz).datetimepicker({format: formater, autoclose:true, todayBtn:1, todayHighlight:true, language:'zh-CN' });
}
